...
fn ray_color(ray: &Ray) -> Color {
    let t = hit_sphere(&Point3::from_slice([0.0, 0.0, -1.0]), 0.5, ray);
    if t > 0.0 {
        let normal = Vec3::new_unit_vec(ray.at(t) - Vec3::from_slice([0.0, 0.0, -1.0]));
        return 0.5 * Color::from_slice([normal.x() + 1.0, normal.y() + 1.0, normal.z() + 1.0]);
    }
